...
export class YearService {
  ...
  async new(dto: createYear): Promise<Year> {
    await this.checkYear(dto.yea)
    const year = await this.year.create({yea: dto.yea})
    await year.save()
  ...
  async update(id: string, dto: createYear): Promise<Year> {
    ...
    const update = await this.year.findByIdAndUpdate(id, {yea: dto.yea})
  ...
  async pushMovie(yearYea: string, movie: Movie): Promise<void> {
    const year = await this.year.findOne({yea: yearYea})
  ...
  private async checkYear(yea: string): Promise<void> {
    const isYear = await this.year.findOne({yea})
    if(isYear) {throw new BadRequestException('Year is created already')}
  }

  private async getYearId(yea: string): Promise<Year> {
    const year = await this.year.findOne({yea})
    if(!year) {throw new NotFoundException('Year is not defined')}
