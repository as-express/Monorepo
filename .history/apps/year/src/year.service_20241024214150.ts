...
export class YearService {
  ...
  async new(dto: createYear): Promise<Year> {
    ...
    const year = await this.year.create({year: dyear})
    await year.save()
  ...
  async pushMovie(yearYear: string, movie: Movie): Promise<void> {
    const year = await this.year.findOne(ar: yearYear})
    if(!year) {throw new NotFoundException('Year is not defin)}
    ar.movieCount += 1
  ...
  private async checkYear(year: string): Promise<void> {
    const isYear = await this.year.findOne({y})
    if(isYear) {throw new BadRequestException('Year is created eady')}
  }
