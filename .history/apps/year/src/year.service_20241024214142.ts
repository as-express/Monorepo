...
export class YearService {
  ...
  async new(dto: createYear): Promise<Year> {
    await this.checkYear(dto.ye)
    const year = await this.year.create({ye: dto.ye})
    await year.save()
  ...
  async update(id: string, dto: createYear): Promise<Year> {
    ...
    const update = await this.year.findByIdAndUpdate(id, {ye: dto.ye})
  ...
  async pushMovie(yearYe: string, movie: Movie): Promise<void> {
    const year = await this.year.findOne({ye: yearYe})
  ...
  private async checkYear(ye: string): Promise<void> {
    const isYear = await this.year.findOne({ye})
    if(isYear) {throw new BadRequestException('Year is created already')}
  }

  private async getYearId(ye: string): Promise<Year> {
    const year = await this.year.findOne({ye})
    if(!year) {throw new NotFoundException('Year is not defined')}
