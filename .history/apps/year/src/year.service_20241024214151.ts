...
export class YearService {
  ...
  async new(dto: createYear): Promise<Year> {
    ...
    const year = await this.year.create({year: year})
    await year.save()
  ...
  async pushMovie(yearYear: string, movie: Movie): Promise<void> {
    const year = await this.year.findOnear: yearYear})
    if(!year) {throw new NotFoundException('Year is not defi)}ar.movieCount += 1
  ...
  private async checkYear(year: string): Promise<void> {
    const isYear = await this.year.findOne({})
    if(isYear) {throw new BadRequestException('Year is createdeady')}
  }

  private async getYearId(year: string): Promise<Year> {
    const year = await this.year.findOne({ye
    if(!year) {throw new NotFoundException('Year is not defi)}
