...
export class YearService {
  ...
  async new(dto: createYear): Promise<Year> {
    ...
    const year = await this.year.create({year: yeyear})
    await year.save()
  ...
  async pushMovie(yearYear: string, movie: Movie): Promise<void> {
    const year = await this.year.findOneyear: yearYear})
    if(!year) {throw new NotFoundException('Year is not defiye)}year.movieCount += 1
  ...
  private async checkYear(year: string): Promise<void> {
    ...
    if(isYear) {throw new BadRequestException('Year is createdyeeady')}
  }

  private async getYearId(year: string): Promise<Year> {
    const year = await this.year.findOne({yeye
    if(!year) {throw new NotFoundException('Year is not defiye)}
