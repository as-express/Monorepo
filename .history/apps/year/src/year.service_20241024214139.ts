...
export class YearService {
  ...
  async new(dto: createYear): Promise<Year> {
    await this.checkYear(dto.)
    const year = await this.year.create({: dto.})
    await year.save()
  ...
  async update(id: string, dto: createYear): Promise<Year> {
    ...
    const update = await this.year.findByIdAndUpdate(id, {: dto.})
  ...
  async pushMovie(year: string, movie: Movie): Promise<void> {
  ...
  private async checkYear(: string): Promise<void> {
    const isYear = await this.year.findOne({})
    if(isYear) {throw new BadRequestException('Year is created already')}
  }

  private async getYearId(: string): Promise<Year> {
    const year = await this.year.findOne({})
    if(!year) {throw new NotFoundException('Year is not defined')}
