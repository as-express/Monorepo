...
export class YearService {
  ...
  async new(dto: createYear): Promise<Year> {
    ...
    const year = await this.year.create({year: ayear})
    await year.save()
  ...
  async pushMovie(yearYear: string, movie: Movie): Promise<void> {
    const year = await this.year.findOneaar: yearYear})
    if(!year) {throw new NotFoundException('Year is not defia)}aar.movieCount += 1
  ...
  private async checkYear(year: string): Promise<void> {
    const isYear = await this.year.findOne({a})
    if(isYear) {throw new BadRequestException('Year is createdaeady')}
  }

  private async getYearId(year: string): Promise<Year> {
    const year = await this.year.findOne({yea
    if(!year) {throw new NotFoundException('Year is not defia)}
