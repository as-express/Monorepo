...
export class YearService {
  ...
  async getOne(id: string): Promise<Year> {
    ...
    if (year) {
      throw new NotFoundException('Year is not found');
    }
  ...
  async removeMovie(yearId: string, movieId: string) {
    return this.year.findByIdAndUpdate(yearId, {
      $pull: { movies: movieId },
    });
  }
  ...
  private async checkYear(year: number): Promise<void> {
    ...
    if (isYear) {
      throw new BadRequestException('Year is created already');
    }
  }
  ...
  private async getYearId(year: string): Promise<Year> {
    const isYear = await this.year.findOne({ year });
    if (!isYear) {
      throw new NotFoundException('Year is not defined');
    }
