...
export class MovieService {
  ...
  async new(dto: movieDto): Promise<any> {
    ...
    this.http.patch(`http://localhost:3001/genre/${dto.genre}`, {
      $push: { movies: movie._id }
    });

    this.http.patch(`http://localhost:3002/year/${dto.genre}`, {
      $push: { movies: movie._id }
    });
  ...
  async delete(id: string): Promise<boolean> {
    const movie = aw this.getOne(id)
    await this.movie.findByIdAndDelete(id)

    this.http.patch(`http://localhost:3001/genre/${movie.genre}`, {
      $pull: { movies: id }
    });

    this.http.patch(`http://localhost:3002/year/${movie.year}`, {
      $pull: { movies: id }
    });
